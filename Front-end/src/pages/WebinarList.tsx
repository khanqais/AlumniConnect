...
//     platform: string;
//     // meetingLink: string;
//     roomId?: string; // backend provides this
//     organizer: {
//         _id: string;
//         name: string;
//         email: string;
...
//         try {
//             await axios.delete(`http://localhost:5000/api/webinars/${eventId}`, {
//                 headers: {
//                     Authorization: `Bearer ${user.token}`,
//                 },
//             });
...
const WebinarList = () => {
    ...
    useEffect(() => {
        ...
        if (user) {
            fetchMyRegistrations();
            if (user.role === 'alumni') fetchMyEvents();
        }
    }, [fetchEvents, fetchMyRegistrations, fetchMyEvents, user]);
            ...
            <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {user?.role === 'alumni' && (
                            <button
                                onClick={() => navigate('/webinar-scheduler')}
                                className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-700 px-4 py-2 text-sm font-medium text-white hover:shadow-lg"
                            >
                                Create Webinar
                            </button>
                        )}
                ...
                <div className="mb-6 flex gap-2 border-b border-gray-200">
                    <button
                        onClick={() => setActiveTab('all')}
                        className={`border-b-2 px-4 py-2 text-sm font-medium transition-colors ${
                            activeTab === 'all' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:border-gray-300 hover:text-gray-900'
                        }`}
                    >
                        All Webinars
                    </button>
                    <button
                        onClick={() => setActiveTab('registered')}
                        className={`border-b-2 px-4 py-2 text-sm font-medium transition-colors ${
                            activeTab === 'registered' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:border-gray-300 hover:text-gray-900'
                        }`}
                    >
                        My Registrations ({myRegistrations.length})
                    </button>
                    {user?.role === 'alumni' && (
                        <button
                            onClick={() => setActiveTab('my')}
                            className={`border-b-2 px-4 py-2 text-sm font-medium transition-colors ${
                                activeTab === 'my' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:border-gray-300 hover:text-gray-900'
                            }`}
                        >
                            My Webinars ({myEvents.length})
                        </button>
                ...
                {loading ? (
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {/* {(activeTab === 'all' ? events : activeTab === 'registered' ? myRegistrations : myEvents).map((event) => (
                            <EventCard
                                key={event._id}
                                event={event}
                                onRegister={handleRegister}
    ...
    return (
        <div className="flex flex-col rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg">
            <div className="mb-4 flex items-start justify-between">
                <span
                    className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                        event.status === 'upcoming' ? 'bg-green-100 text-green-800' : event.status === 'ongoing' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                    }`}
                >
                    {/* {event.status.charAt(0).toUpperCase() + event.status.slice(1)} */}
                    {(event.status ?? '').charAt(0).toUpperCase() + (event.status ?? '').slice(1)}
                ...
                {isFull && <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-800">Full</span>}
            </div>

            <h3 className="mb-2 text-xl font-semibold text-gray-900">{event.webinarName}</h3>
            ...
            {event.skillsCovered?.length > 0 && (
                <div className="mb-4 flex flex-wrap gap-2">
                    {event.skillsCovered.slice(0, 3).map((skill, idx) => (
                        <span key={idx} className="rounded-md bg-blue-50 px-2 py-1 text-xs text-blue-700">{skill}</span>
                    ))}
                    {event.skillsCovered.length > 3 && <span className="rounded-md bg-gray-100 px-2 py-1 text-xs text-gray-700">+{event.skillsCovered.length - 3}</span>}
                </div>
            )}
            ...
            <div className="mt-auto border-t border-gray-100 pt-4 flex flex-col gap-2">
                {canRegister && (
                    <button
                        ...
                        className={`rounded-lg px-4 py-2 text-sm font-medium ${
                            isRegistered ? 'border border-gray-300 bg-white text-gray-700 hover:bg-gray-50' : 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white hover:shadow-lg'
                        }`}
                    >
                        {isRegistered ? 'Unregister' : 'Register'}
                    </button>
                ...
                {event.meetingLink && (isRegistered || isOrganizer) && (
                    <a
                        href={event.meetingLink}
                        target="_blank"
                        rel="noopener noreferrer"
